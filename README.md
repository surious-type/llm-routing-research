...
Notebook `notebooks/experiment_sequence.ipynb` оформляет работу как последовательность «проблема → гипотеза → эксперимент → результат → вывод → следующая гипотеза». Числа в кодовых ячейках читаются непосредственно из сохранённых result artifacts.
